                ...
                if (active && payload && payload.length) {
                  ...
                  return (
                    <div className="p-3 rounded-xl bg-white dark:bg-darkSurface border border-slate-200 dark:border-white/10 shadow-2xl backdrop-blur-md text-xs space-y-1">
                      <div className="font-bold text-slate-900 dark:text-white">{data.name}</div>
                      <div className="text-slate-600 dark:text-slate-400 font-medium">{data.date} • {data.platform}</div>
                      <div className="pt-1 flex items-center justify-between gap-4 font-bold text-sm">
                        <span className="text-blue-600 dark:text-electric-blue">{data.score} Marks</span>
                        <span className="text-purple-700 dark:text-lavender">→ {data.percentile}%ile</span>
                      </div>
                    </div>
                  );
                }
