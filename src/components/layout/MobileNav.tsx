    ...
    <nav className="md:hidden fixed bottom-3 left-3 right-3 sm:left-6 sm:right-6 max-w-md mx-auto z-40 select-none pb-[calc(env(safe-area-inset-bottom,0px))] pointer-events-none animate-slide-up">
      <div
        className={`pointer-events-auto flex items-center justify-between px-2 py-1.5 rounded-[26px] backdrop-blur-2xl border transition-all duration-300 relative ${
          isDark
            ? 'bg-[#090D16]/95 border-white/[0.08] shadow-[0_16px_40px_rgba(0,0,0,0.8),inset_0_1px_0_0_rgba(255,255,255,0.12)]'
            : 'bg-white/95 border-slate-200/90 shadow-[0_16px_40px_rgba(15,23,42,0.10),0_4px_12px_rgba(15,23,42,0.04),inset_0_1px_0_0_rgba(255,255,255,0.95)]'
        }`}
      >
        {/* Top Subtle Ambient Glass Shine Bevel */}
        <div className="absolute top-0 left-6 right-6 h-[1px] bg-gradient-to-r from-transparent via-violet-500/40 dark:via-cyan-400/40 to-transparent pointer-events-none" />
        ...
        <div className="flex items-center justify-center px-1.5">
          <button
            ...
            className="relative w-12 h-12 -translate-y-2.5 rounded-full bg-gradient-to-tr from-[#7C3AED] via-[#6D28D9] to-[#2563EB] text-white shadow-[0_8px_24px_rgba(124,58,237,0.45),0_2px_6px_rgba(0,0,0,0.1)] border-2 border-white dark:border-[#131926] flex items-center justify-center tap-bounce cursor-pointer group active:scale-90 active:translate-y-0 transition-all duration-200"
            title="Add Custom Study Target"
            aria-label="Add Target"
          >
            {/* Ambient Pulse Ring */}
            <span className="absolute -inset-1.5 rounded-full bg-gradient-to-r from-violet-600 via-indigo-500 to-cyan-400 opacity-40 blur-md group-hover:opacity-75 transition-opacity -z-10 animate-pulse" />
