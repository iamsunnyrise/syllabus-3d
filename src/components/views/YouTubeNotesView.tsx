...
export const YouTubeNotesView: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (!apiKey) {
      setError('Please add your Gemini API key in Settings first. Go to Settings → AI Configuration → Paste your free Google Gemini API key.');
      return;
    }
  ...
  }, [url, isValidUrl, metadata, settings, manualTranscript, loadSavedNotes]);
  ...
  const handleAiAction = useCallback(async (action: AiActionType) => {
    ...
    try {
      const result = await executeAiAction(action, textToProcess, apiKey, {
  ...
  }, [selectedText, generatedNotes, settings.language, metadata?.title, currentNoteId, loadSavedNotes]);

  // ── Full Document Translation ──
  const handleTranslateDocument = useCallback(async () => {
    const apiKey = getStoredGeminiApiKey();
    if (!apiKey || !generatedNotes) return;

    const targetLang: NoteLanguage = settings.language === 'hindi' ? 'english' : 'hindi';
    setIsAiActionRunning(true);

    try {
      const translated = await translateFullDocument(generatedNotes, targetLang, apiKey);
  ...
  }, [generatedNotes, settings.language, currentNoteId, loadSavedNotes]);
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-5 animate-view-fade">
        ...
        {error && !showManualPaste && (
          <div className={`rounded-2xl border p-4 flex items-start gap-3 animate-view-fade ${
            isDark
