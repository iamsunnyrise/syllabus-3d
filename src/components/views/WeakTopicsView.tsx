              ...
              >
                ...
                <div className="flex items-center justify-between gap-2 border-b border-slate-100 dark:border-white/5 pb-2.5">
                  ...
                  <div className="flex items-center gap-1.5 min-w-0 flex-1">
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg text-[11px] sm:text-xs font-bold font-sans bg-indigo-50 dark:bg-indigo-500/15 text-indigo-700 dark:text-indigo-300 border border-indigo-200/80 dark:border-indigo-500/30 truncate max-w-[140px] xs:max-w-[170px] sm:max-w-none shrink-0 sm:shrink">
                    ...
                    {chapterName && (
                      <>
                        ...
                        <span className="text-[11px] sm:text-xs font-semibold text-slate-400 dark:text-slate-500 truncate min-w-0" title={chapterName}>
                          {chapterName}
                        </span>
                      </>
                    )}
                  ...
                  {/* Right: Accuracy Badge + Mastered Button (Desktop) */}
                  <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                    {/* Accuracy Badge */}
                    <div className={`px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-lg sm:rounded-xl text-[11px] sm:text-xs font-bold font-sans flex items-center gap-1 sm:gap-1.5 whitespace-nowrap shadow-2xs ${
                      isCritical
                        ? 'bg-rose-50 dark:bg-rose-500/15 text-rose-600 dark:text-rose-400 border border-rose-200/80 dark:border-rose-500/25'
                        : 'bg-amber-50 dark:bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-200/80 dark:border-amber-500/25'
                    }`}>
                    ...
                    {/* Quick Mark Mastered Button - Desktop */}
                    <button
                      onClick={e => handleMarkMastered(e, topic.id)}
                      className="hidden sm:inline-flex items-center gap-1 px-2.5 py-1 rounded-lg sm:rounded-xl bg-emerald-500/10 hover:bg-emerald-500 hover:text-white text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 text-[11px] sm:text-xs font-bold transition-all cursor-pointer active:scale-95 whitespace-nowrap"
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="space-y-1.5 min-w-0">
                    <h3 className="text-sm sm:text-base font-black text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors leading-snug">
                      {topic.name}
                    </h3>
                    <div className="flex items-center gap-2 flex-wrap text-[11px] sm:text-xs font-medium text-slate-500 dark:text-slate-400">
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-100 dark:bg-white/5 border border-slate-200/60 dark:border-white/10 text-slate-700 dark:text-slate-300">
                        Diff: <strong className="font-bold">{topic.difficulty || 'Medium'}</strong>
                      </span>
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-100 dark:bg-white/5 border border-slate-200/60 dark:border-white/10 text-slate-700 dark:text-slate-300">
                        Weight: <strong className="font-bold">{topic.weightage || 'High'}</strong>
                      </span>
                      {mistakes.length > 0 && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-amber-50 dark:bg-amber-500/15 border border-amber-200/70 dark:border-amber-500/25 text-amber-700 dark:text-amber-300 font-bold">
                          <AlertTriangle className="w-3 h-3 text-amber-500 shrink-0" />
                          <span>{mistakes.length} Traps</span>
                          {activeMistakes.length > 0 && (
                            <span className="text-[10px] opacity-80">({activeMistakes.length} Active)</span>
                          )}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto shrink-0 justify-end pt-1 sm:pt-0 border-t sm:border-t-0 border-slate-100 dark:border-white/5">
                    ...
                    {/* Quick Mark Mastered Button - Mobile Only */}
                    <button
                      onClick={e => handleMarkMastered(e, topic.id)}
                      className="sm:hidden h-9 px-3 rounded-xl bg-emerald-500/10 hover:bg-emerald-500 hover:text-white text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 text-[11px] font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer active:scale-95 shrink-0"
                      title="Mark as Mastered"
                    >
                      <Check className="w-3.5 h-3.5 stroke-[2.5]" />
                      <span>Mastered</span>
                    ...
                    <div className="h-9 sm:h-9.5 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-white/10 dark:hover:bg-white/15 flex items-center justify-center gap-1 text-[11px] sm:text-xs font-bold text-blue-600 dark:text-blue-400 transition-all cursor-pointer shrink-0">
                      <span>Inspect</span>
                ...
                {mistakes.length > 0 && (
                  <div className="p-3 sm:p-3.5 rounded-xl sm:rounded-2xl bg-slate-50/90 dark:bg-[#0E101D] border border-slate-200/80 dark:border-white/[0.08] space-y-2">
                    <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase font-mono tracking-wider flex items-center gap-1.5">
                    ...
                    {mistakes.slice(0, 2).map((m, idx) => (
                      <div key={idx} className="flex items-start gap-2 text-xs text-slate-800 dark:text-slate-200">
                        <span className={`px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-mono font-bold uppercase tracking-wider shrink-0 mt-0.5 border ${
                          m.mistakeType === 'conceptual' ? 'bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/25' :
                          m.mistakeType === 'calculation' ? 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/25' :
                          m.mistakeType === 'formula' ? 'bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/25' :
