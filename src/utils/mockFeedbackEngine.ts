...
import { WeakSectionDiagnosis, PerformanceInsight, MockVerdict } from '../types/mockAnalytics';
import { calculateSubjectStats } from './mockAnalyticsEngine';
...
function getSubjectRecommendation(name: SectionName, accuracy: number, _attemptRate: number): string {
...
export function generatePerformanceInsights(mocks: MockTest[]): PerformanceInsight[] {
  ...
  if (subStats.length >= 2) {
    ...
    if (weakest.averageAccuracy < 72) {
      insights.push({
        id: 'weakest-subject',
        type: 'WARNING',
        title: `Negative Marks Spike in ${weakest.label} ⚠️`,
  ...
  if (avgTime > 55) {
    insights.push({
      id: 'time-caution',
      type: 'NEUTRAL',
      title: 'Operating Close to Time Ceiling ⏱️',
