...
import { OverallKPIs, SubjectStat } from '../types/mockAnalytics';
import { THEME_COLORS } from './mockColors';
import { getPerformanceStatus } from './mockCalculations';
  ...
  for (const item of standardSections) {
    ...
    if (data && data.count > 0) {
      ...
      results.push({
        ...
        status,
        color: (THEME_COLORS.subjects as Record<string, string>)[item.name] || THEME_COLORS.electricBlue,
        icon: item.icon
      });
    }
  }
